...
class VSSM(nn.Module):
    def __init__(self, confidence=50, b=4000, patch_size=4, in_chans=3, num_classes=1000, depths=[2, 2, 4, 2], depths_decoder=[2, 9, 2, 2],
                 dims=[96,192,384,768], dims_decoder=[768, 384, 192, 96], d_state=16, drop_rate=0., attn_drop_rate=0., drop_path_rate=0.1,
                 norm_layer=nn.LayerNorm, patch_norm=True,
                 use_checkpoint=False, **kwargs):
        super().__init__()
        self.b = nn.Parameter(torch.tensor(b, dtype=torch.float32))  # Trainable parameter
    ...
    def forward(self, x, confidence):
        ...
        epsilon = 1e-10
        entropy = -torch.sum(softmax_probs * torch.log(softmax_probs + epsilon), dim=1)
        ...
        if confidence == 0:
            ...
            y_thresh = self.b / (100 - confidence)
        ...
        should_reject = (entropy > y_thresh)
        #print(f'current model b value: {self.b}')
...
# Unpacking the model outputs
softmax_probs, preds, should_reject = medmamba_t(data, confidence=confidence_level)
print(softmax_probs.shape)  # Should print [1, 6] (if 6 is the number of classes)
